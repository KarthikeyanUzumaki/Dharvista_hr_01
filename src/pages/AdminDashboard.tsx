...
import { LayoutDashboard, Users, Plus, Trash2, LogOut, FileText, Flame, Menu, Filter, ListFilter } from "lucide-react";
    ...
    <div className="flex h-screen bg-gray-50 flex-col md:flex-row overflow-hidden">
      ...
      <header className="md:hidden bg-white border-b border-gray-200 p-4 flex items-center justify-between z-30 h-20">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-xl text-primary tracking-tight uppercase">DHARVISTA</span>
        </div>
      ...
      <aside className="hidden md:flex w-72 bg-white border-r border-gray-200 flex-col h-full z-10">
        <div className="p-6 border-b border-gray-100 flex flex-col items-center gap-2 text-center">
          ...
          <div>
             <h2 className="text-2xl font-bold text-primary tracking-tight uppercase leading-none">DHARVISTA</h2>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8">
        <div className="max-w-6xl mx-auto">
          ...
          {activeView === 'jobs' ? (
            ...
            <ApplicantsView applicants={applicants} loading={appLoading} updateStatus={updateApplicantStatus} />
          )}
              ...
              <div key={job.id} className="flex flex-col md:flex-row justify-between p-4 border rounded-lg hover:bg-gray-50 transition gap-4">
                <div className="space-y-1 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {job.priority === 'urgent' && <Badge variant="secondary" className="bg-accent text-accent-foreground border-yellow-500 h-5"><Flame className="w-3 h-3 mr-1" /> Urgent</Badge>}
...
function ApplicantsView({ applicants, loading, updateStatus }: any) {
  ...
  const uniqueJobTitles = useMemo(() => {
    const titles = applicants.map((app: any) => app.jobTitle);
    return Array.from(new Set(titles)) as string[];
  }, [applicants]);
  ...
  }, [applicants, selectedJob, selectedStatus]);
